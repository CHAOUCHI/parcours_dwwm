...
Il existe de nombreuse bibliotèques pour générer un JWT, je vais utiliser le paquet npm `jsonwebtoken` et la bibliotèque express pour l'exemple.
...
> Attention, le fait de stocker le token dans le local Storage permet une potentielle attaque XSS. Voir le livre en web application Security. 
> La solution serait de stocker le token dans les cookies HTTP car les cookies sont inaccessibles par la fonction Fetch. 
...
<!-- #### SECURITE - Stocker le token dans le localStorage
Si le token est stoké dans le localStorage il est possible pour un hacker de le récupérer en lisant le localStorage à la suite d'un injection de code JS dans une des pages HTML de notre front-end.

Il peut ensuite envoyer le token d'un client innocent à son propre serveur.

La solution c'est de stocker le token dans les cookies, car ceux-ci ne sont pas accessible en JS et donc inacessible par un fetch. -->
...
> Plus haut je dis *effectuer une deconnexion* mais en réalité le serveur est sans état (il n'a pas consience des utilisateurs qui le consomme); il n'effectue donc pas expréssement de déconnexion.
> Une déconnexion explicite impliquerait l'utilisation des sessions. PHP possède la fonction session_start() pour résoudre ce problème. Aujourd'hui avec la prolifération des API Rest, cette méthode d'authentification devient de plus en plus rare.
...
## Authorization - Sécuriser une route
A chaque requete sensible, le client va devoir fournir le token jwt pour être autoriser à reçevoir la réponse.

Le token ce place dans le header `Authorization` de la requête et est vérifié dans un middleware.

> Je pourrais aussi recevoir le token dans le body de la rquete du client mais etant donné que `fetch()` refuse le body pour la méthode GET je ne pourrais pas fournir le token sur mes routes GET. 
...
```
### Code Serveur - L'api vérifie le token
...
https://github.com/CHAOUCHI/jwt-demo-nodejs.git