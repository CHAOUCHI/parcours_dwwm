...
La première choses à faire est de configurer le fichier `angular.json` pour dire à Angular où doivent se trouver les composants, services et autres classes que nous allons créer.

Dans angular.json vous avez un champs schematics vide qui défini le comportement de angular CLI
```json
"schematics": {
...
> Nous utilisons également le champs `type` pour nommer de façon explicite les composants, services et guards que nous allons créer.
> FAIT TOUJOURS CA POUR TOUT VOS PROJETS ANGULAR ! C'est très important pour la maintenabilité de votre code.
...
Cette variable est accessible de tout les composants de l'application. (voir la section suivante)
